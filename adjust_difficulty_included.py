...
class Blockchain:
    ...
    def __init__(self):
        ...
        self.halving_time = 40
        
        self.nodes = set()
        self.current_target =  0x000000000000000006f0a8000000000000000000000000000000000000000000
        self.max_target = 0x00000000ffff0000000000000000000000000000000000000000000000000000
        self.difficulty_adjust = 5
        self.block_time = 2 # 2 second
    ...
    def create_block(self, proof , previous_hash):
        ...
        if block['index'] % self.difficulty_adjust == 0:
            self.adjust_difficulty(block['timestamp'] , self.chain[-1*self.difficulty_adjust+1]['timestamp'])
        self.transactions = []
        self.transaction_hashes=[]
        self.chain.append(block)
        return block
    
    def adjust_difficulty(self,last,first):
        actual = last - first   
        expected = 2016 * 10 * 60 
        ratio = float(actual) / float(expected)
        
        new_target = (self.current_target * ratio)
        ...
        self.current_target = truncated.rjust(64, '0')
    ...
    def proof_of_work(self , previous_proof):
        # difficulty not added in POW only nonce is encoded to make simple
        new_proof = 1
        check_proof = False
        while check_proof is False:
            hash_operation = hashlib.sha256(str(new_proof**2 - previous_proof).encode()).hexdigest()
            comp = str(self.current_target)
            comp = comp.rjust(len(hash_operation) - len(comp) , '0')
            # for simplicity 0's are compared
...
def mine_block():
    ...
    blockchain.add_transaction(sender = node_address , receiver = 'Yash' , amount = blockchain.block_reward)
